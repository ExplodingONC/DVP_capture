        ...
        if (spi_is_readable(spi_slave))
        {
            ...
            while (gpio_get(FV_PIN))
            {
                ...
                if (spi_is_readable(spi_slave))
                {
                    ...
                    printf("SPI query refused, wait new frame: 0x%02x.\n", command);
                }
            }
            ...
            while (gpio_get(FV_PIN) || dma_channel_is_busy(dma_dvp))
            {
                ...
                {
                    spi_read_blocking(spi_slave, false, &command, 1);
                    printf("SPI query refused, wait frame finish: 0x%02x.\n", command);
                }
            }
            ...
            printf("temp: 0x%4X 0x%4X.\n", frame_buff[0][0][0], frame_buff[0][0][1]);
            printf("      0x%4X 0x%4X.\n", frame_buff[1][0][0], frame_buff[1][0][1]);
